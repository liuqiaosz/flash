package death.def.communicator
{
	...
	import death.def.module.message.event.PixelNetEvent;
	import death.def.module.message.Charset;
	...
	public class PixelNetSocket extends EventDispatcher implements IPixelNetConnection
	{
		...
		public function connect(ip:String,port:int,encode:String = Charset.UTF8):void
		{
			_ip = ip;
			_port = port;
			_charset = encode;
		...
		public function sendMessage(msg:IMsg):void
		{
			...
			{
				_channel.writeBytes(msg.getMessage() as ByteArray);
				_channel.flush();
			}
		}
